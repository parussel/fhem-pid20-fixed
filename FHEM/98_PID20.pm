## \(Id: 98_PID20.pm 29518 2026-05-16 07:15:00Z parussel\)
...
# along with fhem. If not, see <http://gnu.org>.
#
#
# V 1.00.c
...
# V 1.0.0.13 Bugfix: D-portion sign fixed when pidReverseAction is enabled.
# Original logic caused D-portion to act as positive feedback under 
# reverse action. Corrected to provide correct negative feedback damping.
# V 1.0.0.14 Feature: Added pidDPortionFilter attribute for moving average smoothing 
# of the D-portion gradient to prevent oscillation on slow mixing valves.
#
=cut
...
#
# 2026-05-16 1.0.0.13 - Bugfix (parussel):
# D-portion sign fixed when pidReverseAction is enabled.
# Original logic caused D-portion to act as positive feedback under 
# reverse action. Corrected to provide correct negative feedback damping.
#
# 2026-05-16 1.0.0.14 - Feature (parussel):
# Added pidDPortionFilter attribute for moving average smoothing 
# of the D-portion gradient to prevent oscillation on slow mixing valves.
#
######################################################################################
##############
...
{
  my (\(hash) = @_;\)hash->{DefFn}    = 'PID20_Define';
  \(hash->{UndefFn}  = 'PID20_Undef';\)hash->{SetFn}    = 'PID20_Set';
  \(hash->{GetFn}    = 'PID20_Get';\)hash->{NotifyFn} = 'PID20_Notify';
  \(hash->{AttrFn}   = 'PID20_Attr';\)hash->{AttrList} =
...
sub PID20_Define(\$\$\$)
{
  ...
  return if !\$init_done;
  return PID20_Check(\(hash,\)a[2], \(a[3]); }  sub PID20_Check(\)\$\$)
{ my ( \$hash, \(sensorinfo,\)actorinfo ) = @_;
  ...
  if (!\$err) {
    ...
    \(hash->{helper}{actorCommand}  = ( defined(\)cmd) ) ? \(cmd : '';\)hash->{helper}{stopped}       = 0;
    \(hash->{helper}{adjust}        = '';\)modules{PID20}{defptr}{\(name} =\)hash;
...
sub PID20_Notify(\$\$)
{
  ...
  return if (IsDisabled(\(name) \vert{}\vert{} !\)init_done);
...
sub PID20_Set(\$@)
{
  ...
  if (\$cmd eq '?') {
    ...
    PID20_RestartTimer(\(hash,1);   } elsif (\)cmd eq 'stop') {
    return 'Set stop needs a <value> parameter'
      if ( @a != 2 );
    \$hash->{helper}{stopped} = 1;
    PID20_RestartTimer(\(hash,1);   } elsif (\)cmd eq 'restart') {
...
sub PID20_Calc(\$)
{
  ...
  while (1)
  {
    ...
    if ($calcReq)
    {
      ...
      # =========================================================================
      # ÄNDERUNGS-LOG - VERGLEICH ZUM ORIGINAL (D-ANTEIL SIGN FIX)
      # =========================================================================
      ...
      # KORREKTUR: Der deltaGradient muss bei ReverseAction ebenfalls invertiert werden!
      my $workGradient = ( $hash->{helper}{reverseAction} == 1 ) ? -$deltaGradient : $deltaGradient;

      # FEATURE V1.0.0.14: Gleitender Mittelwert-Filter für den D-Anteil
      my $filterDepth = AttrVal($name, "pidDPortionFilter", 1);
      if ($filterDepth > 1) {
          $hash->{helper}{gradientHistory} = [] if (!defined($hash->{helper}{gradientHistory}));
          push(@{$hash->{helper}{gradientHistory}}, $workGradient);
          while (scalar(@{$hash->{helper}{gradientHistory}}) > $filterDepth) {
              shift(@{$hash->{helper}{gradientHistory}});
          }
          my $gradSum = 0;
          $gradSum += $_ foreach (@{$hash->{helper}{gradientHistory}});
          $workGradient = $gradSum / scalar(@{$hash->{helper}{gradientHistory}});
      }

      # calc p-portion
      $pPortion = $workDelta * $hash->{helper}{factor_P};

      # calc d-Portion (NUTZT JETZT DEN KORRIGIERTEN UND GEGLÄTTETEN GRADIENTEN)
      $dPortion = ($workGradient) * $hash->{helper}{calcInterval} * $hash->{helper}{factor_D};
...
sub PID20_Attr($$$$)
{
  ...
  # FEATURE V1.0.0.14: Wenn das Filter-Attribut gelöscht wird, setze den internen Wert zurück auf 1
  if ( $command eq 'del' && $attribute eq 'pidDPortionFilter' )
  {
    $hash->{helper}{dPortionFilter} = 1;
    delete($hash->{helper}{gradientHistory});
  }
...
<h3>PID20</h3>
<ul>
<p><b>Note:</b> This repository contains the community-fixed version (V 1.0.0.14) which fixes the sign inversion bug of the D-portion when <code>pidReverseAction</code> is activated and adds a moving average smoothing filter via <code>pidDPortionFilter</code>.</p>
...
<a id="PID20-attr-pidDPortionFilter"></a>
<li><b>pidDPortionFilter</b> - Number of past gradient samples for moving average smoothing to stabilize a nervous D-portion calculation (e.g. 10); default: 1 (no filtering)</li>
...
<ul>
<li><a href="http://fhem.de">Discussion in FHEM forum</a></li><br/>
<li><a href="http://fhemwiki.de">Information in FHEM wiki</a></li><br/>
</ul>
